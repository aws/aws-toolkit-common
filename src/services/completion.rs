...
pub fn completion(document: &TextDocument, params: CompletionParams) -> Vec<CompletionItem> {
    ...
    rets.push(CompletionItem::new_simple(node_val.to_string(), n.kind().to_string()));

    let node_val2 = n.parent().unwrap().child(0).unwrap().utf8_text(document.contents.as_bytes()).unwrap();
    rets.push(CompletionItem::new_simple(node_val2.to_string(), n.parent().unwrap().child(0).unwrap().kind().to_string()));
