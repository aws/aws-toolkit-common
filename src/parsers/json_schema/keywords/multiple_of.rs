...
pub fn validate_multiple_of(node: &IRNumber, sub_schema: &Value) -> Option<Diagnostic> {
    ...
    if multiple_of_property.is_none() {
        return None;
    }

    let multiple_of_value = get_number(JsonNumbers::Value(multiple_of_property.unwrap()));
    if multiple_of_value.is_none() {
        return None;
    }

    if node.value % multiple_of_value.unwrap() != 0.0 {
        return Some(to_diagnostic(node.start, node.end, format!("!{:#?} is not a multiple of !{:#?}", node.value, multiple_of_value)));
    }
